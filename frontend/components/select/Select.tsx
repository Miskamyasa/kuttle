...
export default function Select({options, onChange, label, defaultValue, disabled = false}: Props): ReactElement {
  ...
  const uniqueId = useMemo(() => `select-${Math.trunc(Math.random() * 10000)}`, [])
  ...
  return (
    <div>
      {label ? <label
        className={"text-xs text-gray1 mb-[2px]"}
        htmlFor={uniqueId}>
        {label}
      </label> : null}
      <div className={selectWrapperClassName}>
        ...
        <div className={"absolute top-[calc(50%-10px)] right-[8px] flex justify-center items-center w-[20px] h-[20px]"}>
